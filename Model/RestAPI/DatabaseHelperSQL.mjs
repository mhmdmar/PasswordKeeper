import pg from 'pg';
import { Response } from '../Utils/ResponseMessage.mjs';
...
class DatabaseHelperSQL {
    ...
    async getUser(email, password) {
        ...
        let user = await this.query(`SELECT * FROM "Users" WHERE "email" = '${email}' AND  "password" = '${password}';`);
    ...
    async updateUser(email, newUser) {
        if (!email || !newUser) {
            return new Response(200, false, messages.errors.invalidArguments);
        }
        ...
        const result = await this.query(`UPDATE "Users" SET username='${username}', password='${password}', email='${userEmail}', permission='${permission}' WHERE email='${email}';`);
        if (!result) {
    ...
    async insertUser(username, password, email, passwordsList = [], permission = 3) {
        ...
        const result = await this.query(`INSERT INTO "Users"(username, password, email, permission) VALUES ('${username}', '${password}', '${email}', ${permission});`);
        if (!result) {
            return new Response(500, false, messages.errors.serverError, null);
        }
        if (passwordsList && passwordsList.length) {
            ...
            stringifiedPasswords = stringifiedPasswords.slice(0, stringifiedPasswords.length - 1);
            const result = await this.query(`INSERT INTO "Passwords"( username, password, domain, "email") VALUES ${stringifiedPasswords};`);
            if (!result) {
                new Response(200, true, messages.warning.invalidArguments, JSON.stringify(passwordsList));
            }
        }
    ...
    // Passwords Table
    async _setLatestId() {
        const idsList = (await this.query(`SELECT id FROM "Passwords" ORDER BY "id"`)).map(idObj => idObj.id);
        this.latestId = idsList[idsList.length - 1] + 1;
    }
    async getUserPasswords(email) {
        return this.query(`SELECT username,password,domain,id FROM "Passwords" WHERE "email" = '${email}' ORDER BY id`);
    }
    async addPasswordItem(email, password, newPassword) {
        const id = this.latestId++;
        const result = await this.query(`INSERT INTO "Passwords" (username, password, domain, "email", id) VALUES ('${newPassword.username}'
    ...
    async updatePasswordItem(email, password, newPassword) {
        if (!email || !password || newPassword === undefined || newPassword.id === undefined) {
            return new Response(200, false, messages.errors.invalidArguments);
        }
        const id = newPassword.id;
        const result = await this.query(`UPDATE "Passwords" SET username='${newPassword.username}', password='${newPassword.password}', domain='${newPassword.domain}' WHERE "id"='${id}';`);
        return new Response(200, result, result ? messages.success.update : messages.warning.userDoesntExist, newPassword);
    }
